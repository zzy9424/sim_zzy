...
class VSSMAEnv(VSSBaseEnv):
    ...
    def _calculate_reward_and_done(self):
        ...
        if self.frame.ball.x > (self.field.length / 2):
            ...
            if self.last_frame is not None:
                grad_ball_potential, closest_move,move_reward,energy_penalty,speed_reward = 0,0,0,0,0
                ...
                if not self.first_step and dead_robot_count == self.n_robots_control and stuck_terminal:
                    done = True
                    for i in range(self.n_robots_control):
    ...
    def _get_initial_positions_frame(self):
        ...
        def x_blue():
            return random.uniform(-field_half_length + 0.1,0)

        def x_yellow():
            return random.uniform(0,field_half_length - 0.1)
...
class VSSMAOpp(VSSMAEnv):
    ...
    def load_opp(self):
        self.opps = []
        try:
            with open(os.path.dirname(os.path.realpath(__file__)) + f'/opponent/args.npy', 'rb') as f:
            ...
            print(f"No ckp found. Will use random opponents.ckp_path:{ckp_path}")
