...
class TwoDoorEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        self.grid.wall_rect(0, 0, 2 * self.size-1, self.size)
    ...
    def step(self, action):
        ...
        if blue_door_opened_after and red_door_opened_after:
                # Success
                reward = self._reward()
                terminated = True
        elif blue_door_opened_before and not blue_door_opened_after or red_door_opened_before and not red_door_opened_after:
            reward = -self.ckp_reward
        elif  not blue_door_opened_before and blue_door_opened_after or not red_door_opened_before and red_door_opened_after:
            self.ckp_reward = self._reward()/2
            reward = self.ckp_reward
